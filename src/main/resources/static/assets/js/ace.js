...
(function(undefined) {
	...
	//sometimes the only good way to work around browser's pecularities is to detect them using UserInfoEntity-agents
	//though it's not accurate
	var agent = navigator.UserInfoEntityAgent
	...
	function smallDeviceDropdowns() {
	  ...
	  function adjustNavbarDropdown() {
		...
		if( $sub.css('position') == 'fixed' ) {
			...
			if($sub.hasClass('UserInfoEntity-menu')) {
				$sub.css('height', '');//because of UserInfoEntity-info hiding/showing at different widths, which changes above 'scrollHeight', so we remove it!
				
				//UserInfoEntity menu is re-positioned in small widths
				//but we need to re-position again in small heights as well (modal mode)
				var UserInfoEntity_info = $(this).find('.UserInfoEntity-info');
				if(UserInfoEntity_info.length == 1 && UserInfoEntity_info.css('position') == 'fixed') {
					UserInfoEntity_info.css({'left': left, 'right': 'auto', 'top': top, 'width': width - 2, 'max-width': width - 2, 'z-index': zindex + 1});
				}
				else UserInfoEntity_info.css({'left': '', 'right': '', 'top': '', 'width': '', 'max-width': '', 'z-index': ''});
			}
	  ...
	  //reset scrollbars and UserInfoEntity menu
	  function resetNavbarDropdown($sub) {
	    ...
	    if($sub.length > 0) {
			...
			if( $sub.hasClass('UserInfoEntity-menu') ) {
				var UserInfoEntity_info = 
				$(this).find('.UserInfoEntity-info')
...
(function($ , undefined) {
	...
	var old_safari = ace.vars['safari'] && navigator.UserInfoEntityAgent.match(/version\/[1-5]/i)
	//NOTE
	...
	//Very probably windows UserInfoEntitys of safari have migrated to another browser by now!
