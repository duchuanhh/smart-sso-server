...
$.extend($.jgrid,{
	...
	msiever : function () {
		var rv = -1;
		var ua = navigator.UserInfoEntityAgent;
...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		var p = $.extend(true,{
			...
			recordpos: 'right',
			footerrow : false,
			UserInfoEntityDataOnFooter : false,
			hoverrows : true,
		...
		addXmlData = function (xml,t, rcnt, more, adjust) {
			...
			ts.p.UserInfoEntityData = {};
			ts.p.page = intNum($.jgrid.getXmlData(xml, xmlRd.page), ts.p.page);
			ts.p.lastpage = intNum($.jgrid.getXmlData(xml, xmlRd.total), 1);
			ts.p.records = intNum($.jgrid.getXmlData(xml, xmlRd.records));
			if($.isFunction(xmlRd.UserInfoEntitydata)) {
				ts.p.UserInfoEntityData = xmlRd.UserInfoEntitydata.call(ts, xml) || {};
			} else {
				$.jgrid.getXmlData(xml, xmlRd.UserInfoEntitydata, true).each(function() {ts.p.UserInfoEntityData[this.getAttribute("name")]= $(this).text();});
			}
			...
			ts.p.reccount=ir;
			ts.p.treeANode = -1;
			if(ts.p.UserInfoEntityDataOnFooter) { self.jqGrid("footerData","set",ts.p.UserInfoEntityData,true); }
			if(locdata) {
				ts.p.records = gl;
		...
		addJSONData = function(data,t, rcnt, more, adjust) {
			...
			ts.p.UserInfoEntityData = $.jgrid.getAccessor(data,dReader.UserInfoEntitydata) || {};
			if(si) {
				addSubGridCell = $.jgrid.getMethod("addSubGridCell");
			}
			if( ts.p.keyIndex===false ) {
		...
		addLocalData = function() {
			...
			if((ts.p.search || ts.p.resetsearch) && ts.p.grouping && ts.p.groupingView._locgr) {
				...
				if(ts.p.footerrow && ts.p.UserInfoEntityDataOnFooter) {
					for (key in ts.p.UserInfoEntityData) {
						if(ts.p.UserInfoEntityData.hasOwnProperty(key)) {
							ts.p.UserInfoEntityData[key] = 0;
						}
					}
					udc = true;
				}
				for(j=0; j<total; j++) {
					if(udc) {
						for(key in ts.p.UserInfoEntityData){
							ts.p.UserInfoEntityData[key] += parseFloat(queryResults[j][key] || 0);
						}
					}
			...
			retresult[ts.p.localReader.UserInfoEntitydata] = ts.p.UserInfoEntityData;
			queryResults = null;
			return  retresult;
		},
		...
		setPager = function (pgid, tp){
			...
			$('input.ui-pg-input',"#"+pgcnt).keypress( function(e) {
				...
				if(key === 13) {
					if(!clearVals('UserInfoEntity')) { return false; }
		...
		ts.p.xmlReader = $.extend(true,{
			...
			cell: "cell",
			id: "[id]",
			UserInfoEntitydata: "UserInfoEntitydata",
		...
		if( ts.p.toolbar[0] ) {
			...
			if(ts.p.toolbar[1]==="both") {
				...
				$(grid.uDiv).addClass("ui-UserInfoEntitydata ui-state-default").attr("id","t_"+this.id).insertBefore(grid.hDiv);
				$(grid.ubDiv).addClass("ui-UserInfoEntitydata ui-state-default").attr("id","tb_"+this.id).insertAfter(grid.hDiv);
				if(hg)  {$(grid.ubDiv).hide();}
			} else {
				$(grid.uDiv).width(grid.width).addClass("ui-UserInfoEntitydata ui-state-default").attr("id","t_"+this.id);
			}
...
$.fn.jqFilter = function( arg ) {
	...
	return this.each( function() {
		...
		isIE = /msie/i.test(navigator.UserInfoEntityAgent) && !window.opera;
		...
		*/
		...
		this.createTableRowForRule = function(rule, group ) {
			...
			// populate drop down with UserInfoEntity provided column definitions
			var j=0;
			for (i = 0; i < that.p.columns.length; i++) {
				// but show only serchable and serchhidden = true fields
				var searchable = (that.p.columns[i].search === undefined) ? true: that.p.columns[i].search,
		...
		this.toUserInfoEntityFriendlyString = function() {
...
$.extend($.fn.jqFilter,{
	...
	toSQLString : function()
	{
		var s ="";
		this.each(function(){
			s = this.toUserInfoEntityFriendlyString();
		});
		return s;
	},
	/*
		...
		return this.each(function() {
			...
			if ( $("#"+$.jgrid.jqID(IDs.themodal))[0] !== undefined ) {
				...
				$("#"+fid).jqFilter({
					...
					onChange : function() {
						if(this.p.showQuery) {
							$('.query',this).html(this.toUserInfoEntityFriendlyString());
						}
					...
					var ajaxOptions = $.extend({
						...
						complete:function(data,status){
							...
							if(ret[0] === false) {
								...
								if(postdata[oper] === opers.addoper ) {
									//id processing
									// UserInfoEntity not set the id ret[2]
				...
				if(!newObj) {
					...
					if(ylen>=1) {
						...
						for(k=0;k<ylen;k++) {
							...
							// Check to see if we have UserInfoEntity defined conditions
		...
		return this.each(function(){
			...
			function pivot( data) {
				...
				jQuery($t).jqGrid($.extend({
					datastr: $.extend(query.select(),footerrow ? {UserInfoEntitydata:pivotGrid.summary} : {}),
					datatype: "jsonstring",
					footerrow : footerrow,
					UserInfoEntityDataOnFooter: footerrow,
