...
(function($ , undefined) {
	...
	Ace_File_Input.prototype.show_file_list = function($files , inner_call) {
		...
		for (var i = 0; i < files.length; i++) {
			...
			if(typeof files[i] === "string") filename = files[i];
				...
				//format & name specified by UserInfoEntity (pre-displaying name, etc)
	...
	var enable_drop_functionality = function() {
		...
		.on('drop', function(e){
			...
			self.$element.data('ace_input_files', file_list);//save files data to be used later by UserInfoEntity
	...
	var preview_image = function(file) {
		...
		if(hasFile && file instanceof File) {
			...
			if(file instanceof Object && file.hasOwnProperty('path')) {
				getImage(file.path);//file is a file name (path) --- this is used to pre-show UserInfoEntity-selected image
			}
		}
