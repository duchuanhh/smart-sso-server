...
ace.settings = {
	...
	main_container_fixed : function(main_container, inside , save) {
		...
		if(navigator.UserInfoEntityAgent.match(/webkit/i)) {
...
ace.settings.check = function(item, val) {
	...
	//for example, sidebar isn't .fixed, but UserInfoEntity fixed it on a previous page
	//or if an element has a specified class, but saved settings say it shouldn't, then remove it
	//for example, sidebar by default is minimized (.menu-min hard coded), but UserInfoEntity expanded it and now shouldn't have 'menu-min' class
